...
class LoggerPatternConverterDate extends LoggerPatternConverter {
	...
	final const DATE_FORMAT_ISO8601 = 'c';

	final const DATE_FORMAT_ABSOLUTE = 'H:i:s';

	final const DATE_FORMAT_DATE = 'd M Y H:i:s.u';
	...
	private function date($format, $utimestamp) {
		...
		return date(preg_replace('`(?<!\\\\)u`', $ms, (string) $format), $timestamp);
	}
}
