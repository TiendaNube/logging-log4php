...
class LoggerPatternParser {
	...
	public function parse() {
		...
		$count = preg_match_all($this->regex, (string) $this->pattern, $matches, PREG_OFFSET_CAPTURE);
		if ($count === false) {
		...
		foreach($matches[0] as $key => $item) {
			...
			if ($start > $prevEnd) {
				$literal = substr((string) $this->pattern, $prevEnd, $start - $prevEnd);
		...
		if ($end < strlen((string) $this->pattern)) {
			$literal = substr((string) $this->pattern, $end);
			$this->addLiteral($literal);
		}
